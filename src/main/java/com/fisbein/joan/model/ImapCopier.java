...
public class ImapCopier implements Runnable, Closeable {
  ...
  /**
   * Open a connection to the source Store from where the messages will be copied when <code>copy</code> method will be executed
   *
  ...
  /**
   * Open a connection to the target Store where the messages will be copied when <code>copy</code> method will be executed
   *
   * @param storeType Type of Store (imap, imaps, pop3, pop3s)
   * @param host      Server host name
   * @param user      User
      ...
      if (isType(sourceFolder, Folder.HOLDS_MESSAGES)) {
        ...
        if (sourceFolder.getMessageCount() != targetFolder.getMessageCount()) {
          ...
          while (messagesDate.isBefore(toDate)) {
            ...
            log.debug(
              "Copying " + notCopiedMessages.length + " messages from " + sourceFolder.getFullName() + " Folder for " + messagesDate);
  ...
  private Message[] getNotCopiedMessages(Folder sourceFolder, Folder targetFolder, LocalDate date) throws MessagingException {
    ...
    openFolderIfNeeded(sourceFolder, Folder.READ_ONLY);
    List<Message> sourceMessages = Arrays.asList(
      sourceFolder.search(new AndTerm(new SentDateTerm(ComparisonTerm.GE, startDate), new SentDateTerm(ComparisonTerm.LT, endDate))));

    if (!sourceMessages.isEmpty()) {

      openFolderIfNeeded(targetFolder, Folder.READ_ONLY);
      Message[] targetMessages = targetFolder.search(
        new AndTerm(new SentDateTerm(ComparisonTerm.GE, startDate), new SentDateTerm(ComparisonTerm.LT, endDate)));
